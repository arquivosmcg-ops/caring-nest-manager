...
DROP POLICY IF EXISTS "certificado proprio" ON public.certificados_digitais;
CREATE POLICY "certificado proprio" ON public.certificados_digitais
  FOR ALL TO authenticated
  USING (usuario_id = auth.uid())
  WITH CHECK (usuario_id = auth.uid());

DROP TRIGGER IF EXISTS trg_certificados_updated_at ON public.certificados_digitais;
CREATE TRIGGER trg_certificados_updated_at
  BEFORE UPDATE ON public.certificados_digitais
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

ALTER TABLE public.assinaturas
  ADD COLUMN IF NOT EXISTS certificado_tipo text,
  ADD COLUMN IF NOT EXISTS certificado_ac_emissor text,
  ADD COLUMN IF NOT EXISTS protocolo_assinatura text,
...
CREATE OR REPLACE FUNCTION public.registrar_assinatura(
  ...
  _certificado jsonb DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  IF _metodo NOT IN ('pin','senha','icp_a1','icp_a3') THEN
  ...
  IF _metodo IN ('icp_a1','icp_a3') THEN
    IF _certificado IS NULL OR COALESCE(_certificado->>'protocolo','') = '' THEN
      RAISE EXCEPTION 'Dados do certificado digital ausentes';
    END IF;
...
REVOKE ALL ON FUNCTION public.registrar_assinatura(text, uuid, text, text, jsonb, text, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.registrar_assinatura(text, uuid, text, text, jsonb, text, jsonb) TO authenticated;