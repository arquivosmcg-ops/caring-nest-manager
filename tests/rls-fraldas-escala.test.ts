...
function politicas(tabela: string): Pol[] {
  const out = execSync(
    `psql -At -F '|~|' -c "select cmd, roles::text, regexp_replace(coalesce(qual,''),'[[:space:]]+',' ','g'), regexp_replace(coalesce(with_check,''),'[[:space:]]+',' ','g') from pg_policies where schemaname='public' and tablename='${tabela}'"`,
    { encoding: "utf8" },
  );
